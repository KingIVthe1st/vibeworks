...
const particleFragment = /* glsl */ `
  ...
  void main() {
    ...
    gl_FragColor = vec4(color * energy, alpha * vEdgeAlpha);
  }
`
...
const dustVertex = /* glsl */ `
  ...
  void main() {
    ...
    worldPosition.x += uFieldOffset * (1.0 - uPortrait);
...
const dustFragment = /* glsl */ `
  ...
  void main() {
    ...
    gl_FragColor = vec4(vec3(0.64, 0.57, 1.0), mote * vOpacity * vEdgeAlpha);
  }
`
...
const voidFragment = /* glsl */ `
  ...
  void main() {
    ...
    float nebula = 1.0 - smoothstep(0.04, 0.62, length(uv - vec2(0.28, 0.02)));
    color += vec3(0.486, 0.361, 1.0) * nebula * 0.05;
...
function createDust(stage: Stage): { mesh: InstancedMesh; material: ShaderMaterial } {
  const count = Math.round(400 * stage.tier.instances * (stage.tier.name === 'high' ? 2 : 1))
  const moteSize = stage.tier.name === 'high' ? 0.0286 : 0.022
  const geometry = new PlaneGeometry(moteSize, moteSize)
  ...
  const material = new ShaderMaterial({
    ...
    uniforms: {
      ...
      uFieldOffset: { value: stage.portrait ? 0 : 1.35 },
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (nearParticle) {
      ...
      const rightBiased = stage.tier.name === 'high' && random() < 0.68
      dummy.position.set(
        rightBiased ? 0.8 + random() * 6.2 : (random() - 0.5) * 14,
        (random() - 0.5) * 8,
    ...
    opacity[i] = stage.tier.name === 'high' ? 0.1 + random() * 0.12 : 0.04 + random() * 0.08
  ...
  return {
    ...
    init(stage) {
      ...
      particleMaterial = new ShaderMaterial({
        ...
        uniforms: {
          uTime: { value: 0 },
          uSize: { value: (stage.portrait ? 44 : 55) * (stage.tier.name === 'high' ? 1.15 : 1) },
      ...
      onStageFrame(stage, (time) => {
        ...
        blueprintCenter.set(portrait ? 0 : 1.5, portrait ? -0.34 : -1.08, portrait ? -0.35 : 0)
        ...
        particleMaterial.uniforms.uSize.value = (portrait ? 44 : 55) * (stageRef.tier.name === 'high' ? 1.15 : 1)
        ...
        dustMaterial.uniforms.uFieldOffset.value = portrait ? 0 : 1.35
